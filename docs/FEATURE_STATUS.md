...
> ## 📌 기준 고정
>
> **이 표는 `main` 브랜치 커밋 `44f4a92` 의 상태만 나타냅니다.**
>
> - `완료` 는 **`main` 의 코드에 있고 `main` 에서 실행한 테스트가 확인한 것**만 붙였습니다. 추측으로 붙이지 않았습니다.
> - **미병합 브랜치에 구현이 있어도 `main` 상태는 바꾸지 않았습니다.**
>   예를 들어 대시보드 집계 오류는 `fix/core-session-flow` 에 수정이 있지만, `main` 기준으로는 여전히 **부분 완료**입니다.
> - 미병합 브랜치의 기능은 [`PREVIEW_BRANCHES.md`](./PREVIEW_BRANCHES.md) 를 보세요. 그 기능들은 **Production 에 없습니다.**
> - 표의 `테스트` 열 괄호 숫자는 그 파일의 실행 테스트 개수입니다.
> - 수동 확인이 필요한 항목은 `수동 확인 필요` 로 적었고, **자동 테스트가 확인했다는 뜻이 아닙니다.**
...
| 종료 후 이동 경로 | **부분 완료** | `/stretch` | — | `건너뛰기`·`결과 보기` 가 `/result/demo` 로 이동 — 현재 세션 id 를 유지하지 않음 | 세션 id 전달 — `fix/core-session-flow` 에 수정 있음(미병합) |
...
| 대시보드 오늘의 기록 | **부분 완료** | `/` | — | `집중` 값이 `0:00` 으로 하드코딩 | 오늘 날짜 기준 합산 — `fix/core-session-flow` 에 수정 있음(미병합) |
| 대시보드 최근 세션 누적 | **부분 완료** | `/` | — | `완료 세션 수 × 25분` 으로 계산 — 15/50분 세션이 왜곡됨 | `sessionHistoryStore` 합산으로 교체 — `fix/core-session-flow` 에 수정 있음(미병합) |
...
> 위 개수는 **`main` 44f4a92 기준**입니다. 미병합 브랜치의 구현은 세지 않았습니다.

---

## 16. 🌿 Preview 구현 — main 미병합 (참고)

아래는 **`main` 에 없고 Production 에도 없는** 기능입니다.
`fix/core-session-flow` (tip `02a430e`, main 대비 2커밋) 에 구현되어 있습니다.
**위 §1~§14 표의 상태에는 반영하지 않았습니다.**

| 기능 | 상태 | 브랜치 |
|---|---|---|
| 학습 모드 시스템 (기본 3 + 내 모드 최대 3) | Preview · main 미병합 | `fix/core-session-flow` |
| 모드별 마감 괴물 (북몽이 · 늘몽이 · 꼬몽이) | Preview · main 미병합 | 〃 |
| 캘리브레이션 v3 (프레이밍 1.5s + 벽시계 5s + 40표본 + 5버킷) | Preview · main 미병합 | 〃 |
| 다중 자세 기준 프로필 (`profiles[]`) | Preview · main 미병합 | 〃 |
| 세션 중 회복 휴식 (`beginRest`/`endRest`) | Preview · main 미병합 | 〃 |
| 스트레칭 출발지별 복귀 경로 | Preview · main 미병합 | 〃 |
| 오늘의 집중 시간 실제 집계 (`todayFocus`) | Preview · main 미병합 | 〃 |
| 사용자 지정 세션 길이 (5~120분 / 0~30분) | Preview · main 미병합 | 〃 |
| 친구 방 4단계 준비 게이트 | Preview · main 미병합 | 〃 |
| 친구 방 공동 화면 `CoopArena` | Preview · main 미병합 | 〃 |
| 친구 방 재접속 · 장착 아이템 공유 · 응원 5초 쿨다운 | Preview · main 미병합 | 〃 |
| 회복 공격 3.2초 연출 시퀀스 | Preview · main 미병합 | 〃 |
| 세션 이탈 가드 (뒤로가기·새로고침 확인) | Preview · main 미병합 | 〃 |
| 용어 변경 `리셋` → `회복 휴식` | Preview · main 미병합 | 〃 |

검증 범위·병합 전 확인 사항은 [`PREVIEW_BRANCHES.md`](./PREVIEW_BRANCHES.md) 를 보세요.
그 브랜치에는 **Supabase 수동 마이그레이션이 선행되어야 하는 변경**이 포함되어 있습니다.

---

## 17. 가장 먼저 손봐야 할 것 (main 기준 · 영향도 순)
