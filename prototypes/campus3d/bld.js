...
export function union(g, opt = {}) {
  ...
  /* ── 1층 — 앞면만 유리 ──
     전 판은 1층 **덩어리 전체**가 유리였습니다. 밤에 창을 켜자 그 부피가
     통째로 발광해서 학생회관 혼자 등불이 됐습니다 — 옆도 뒤도 유리일
     이유가 없고, 실제 학생회관도 광장을 보는 면만 트여 있습니다.
     몸통은 벽으로 두고 앞면 두 짝만 유리로 남깁니다. */
  box(g, W - .5, G, D - .3, .1, M(P.wall), 0, Y + G / 2, 0);
  box(g, W - .3, .22, D - .1, .06, M(P.trim), 0, Y + .06, 0);
  box(g, W - .3, .3, D - .1, .07, M(P.trim), 0, Y + G, 0);
  [-1, 1].forEach((s) => box(g, 4.1, G - .46, .16, .04, M(P.glass, .16),
                             s * 4.4, Y + G / 2 + .04, (D - .3) / 2 + .02));
  /* 옆과 뒤는 보통 창 둘씩 — 기둥(z ±1.0 · ±3.2)을 비켜 갑니다 */
  [-2.1, 2.1].forEach((z) => [-1, 1].forEach((sx) =>
    win(g, P, sx * ((W - .5) / 2 + .02), Y + G * .55, z, sx * Math.PI / 2, 1.1, 1.0)));
  [-4.4, 0, 4.4].forEach((x) =>
    win(g, P, x, Y + G * .55, -(D - .3) / 2 - .02, Math.PI, 1.3, 1.0));
  ...
  /* 차양 기둥 — 반지름 .12 는 캠퍼스 배율 2.2 를 곱해도 지름 0.53m 입니다.
     폭 32m 짜리 차양을 연필 넷이 받치고 있는 꼴이라, 무게가 안 맞아서
     차양이 떠 보였습니다. 굵히고 받침판을 답니다. */
  /* 기둥 밑에 포장 한 장 — 안 깔면 기둥 넷이 잔디 위에 꽂힙니다 */
  box(g, W + .8, .12, 3.4, .05, M(P.stone, .8), 0, Y - .02, CZ + .6);
  [-5.6, -1.9, 1.9, 5.6].forEach((x) => {
    cyl(g, .22, .25, G + .32, 12, M(P.trim, .5), x, Y + (G + .32) / 2, CZ + 1.2);
    cyl(g, .34, .40, .16, 12, M(P.stoneDark, .7), x, Y + .06, CZ + 1.2);
  });

  /* ── 위 두 층 — 가로 띠창 ── */
  box(g, W - 1.0, U, D - .8, .12, M(P.wall), 0, Y + G + U / 2, 0);
  box(g, W - .7, .26, D - .5, .07, M(P.trim), 0, Y + G + U * .5, 0);
  /* 창을 다섯 짝 폭 2.0 에서 **네 짝 폭 1.5** 로. 면 너비 13 에 유리가
     10 이면 77% 라, 밤에 층 전체가 띠 조명이 됐습니다. 6 이면 46% —
     띠창의 인상은 남고 벽이 벽으로 보입니다. */
  [Y + G + U * .26, Y + G + U * .76].forEach((wy) => {
    [-4.8, -1.6, 1.6, 4.8].forEach((x) => {
      win(g, P, x, wy, (D - .8) / 2 + .02, 0, 1.5, .85);
      win(g, P, x, wy, -(D - .8) / 2 - .02, Math.PI, 1.5, .85);
    });
    [-2.3, 2.3].forEach((z) => [-1, 1].forEach((sx) =>
      win(g, P, sx * ((W - 1.0) / 2 + .02), wy, z, sx * Math.PI / 2, 1.5, .85)));
  });
  ...
  /* 탑도 바닥부터 꼭대기까지 통유리였습니다. 1층 높이만큼 돌 기단을
     두어 **떠 있는 유리 통**으로 만듭니다 — 밤에 발광하는 부피가 그만큼
     줄고, 유리가 땅에 박혀 있지 않아 탑으로 읽힙니다. */
  const TH = TOP - Y - G + .5;
  cyl(g, RR + .06, RR + .14, G, 26, M(P.wallLight, .5), RX, Y + G / 2, RZ);
  cyl(g, RR, RR + .1, TH, 26, M(P.glass, .16), RX, Y + G + TH / 2, RZ);
  for (let i = 0; i < 10; i++) {
    const a = (i / 10) * Math.PI * 2;
    box(g, .18, TH, .2, .04, M(P.wallLight),
        RX + Math.sin(a) * (RR + .04), Y + G + TH / 2, RZ + Math.cos(a) * (RR + .04));
  }
...
export function shop(g, opt = {}) {
  ...
  /* 접힌 차양 천 한 뭉치 — 앞의 줄무늬가 여기 여분으로 있습니다.
     바닥에 눕혀 두었더니 뒷마당에 **빨간 통나무**가 떨어져 있는 것으로
     보였습니다. 벽에 비스듬히 기대 세웁니다 — 치워 둔 물건은 눕히지
     않고 세워 둡니다. 아래에 받침 하나를 깔아 흙에 안 박히게 합니다. */
  const roll = cyl(g, .20, .22, 1.7, 10, M(0xE8483C, .7), 3.0, Y + .82, -D / 2 - .42);
  roll.rotation.x = -.22; roll.rotation.z = .14;
  box(g, .7, .16, .5, .05, M(0x8A6440, .82), 3.0, Y + .08, -D / 2 - .5);
