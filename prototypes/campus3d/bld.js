...
export function union(g, opt = {}) {
  ...
  sign(g, '학생회관', 0, Y + G + 1.02, (D - .8) / 2 + .72, 4.8, .96, '#43A05C', '#FFFFFF');
}
...
export function arcade(g, opt = {}) {
  ...
  box(g, W - .1, .34, D - .1, .1, M(P.roofDark, .7), 0, yT + .3, 0);
  /* 레퍼런스의 세로형 오락실 실루엣을 저작권 장식 없이 재해석합니다.
     보라 타워·티켓 띠·별 토큰으로 멀리서도 게임 시설로 읽힙니다. */
  box(g, 5.7, 1.9, 4.2, .16, M(0xEEE8F8, .55), 0, yT + 1.15, -.25);
  box(g, 5.95, .25, 4.45, .08, M(0x6E58A7, .56), 0, yT + 2.12, -.25);
  box(g, 5.15, .36, .28, .08, M(0xFFD46F, .48), 0, yT + 1.55, 1.94);
  [-1.75, -.88, 0, .88, 1.75].forEach((x, i) => {
    const token = new THREE.Mesh(new THREE.CylinderGeometry(.19, .19, .10, 16),
      M([0xF47A6A, 0x6FC6D9, 0xFFD46F][i % 3], .45));
    token.rotation.x = Math.PI / 2; token.position.set(x, yT + .9, 1.91); token.castShadow = true; g.add(token);
  });
  /* 정면 양옆의 짧은 차양 — 문은 가리지 않습니다. */
  [-2.75, 2.75].forEach((x) => {
    for (let i = 0; i < 5; i++) box(g, .48, .13, .92, .05,
      M(i % 2 ? 0x7862B4 : 0xFFF0B5, .57), x - .96 + i * .48, 2.02, D / 2 + .48);
  });
...
export function shop(g, opt = {}) {
  ...
  sign(g, '동아리 상점', 0, yT + 2.55, D / 2 + .42, 5.2, 1.25, '#C4553F', '#FFF3DC');
