...
export function arcade(g, opt = {}) {
  ...
  /* 티켓 띠와 토큰 — 지붕에서 내려 입구 옆 벽으로. 사람 눈높이에 있어야
     읽힙니다. 지붕 위에서는 위에서 내려다보는 각도에만 보였습니다. */
  box(g, 3.0, .30, .16, .06, M(0xFFD46F, .48), 0, Y + .62, D / 2 + .10);
  [-1.05, -.35, .35, 1.05].forEach((x, i) => {
    const token = new THREE.Mesh(new THREE.CylinderGeometry(.20, .20, .10, 16),
      M([0xF47A6A, 0x6FC6D9, 0xFFD46F][i % 3], .45));
    token.rotation.x = Math.PI / 2;
    token.position.set(x, Y + .62, D / 2 + .20);
  ...
  /* 조이스틱 — 지붕 위에 있던 것을 **땅으로 내립니다.**
     지붕에 두면 주사위와 실루엣이 겹쳐 꼭짓점이 둘이 되고, 무엇이 주인공인지
     흐려집니다. 입구 옆에 사람 키만 하게 세우면 "여기 들어가면 게임한다" 가
     들어가기 전에 읽힙니다. */
  box(g, .9, .28, .9, .08, M(0x6E58A7, .6), 3.1, Y + .14, D / 2 + .95);
  cyl(g, .14, .17, 1.15, 10, M(P.trim, .5), 3.1, Y + .85, D / 2 + .95);
  const knob = new THREE.Mesh(new THREE.SphereGeometry(.32, 16, 12), M(0xE8483C, .4));
  knob.position.set(3.1, Y + 1.52, D / 2 + .95); knob.castShadow = true; g.add(knob);
}
