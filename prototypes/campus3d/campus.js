...
export function buildCampus(scene) {
  ...
  /* 둥근 분수를 **네모로** 막고 있었습니다. 20.6 짜리 정사각형이라
     ...
     모서리마다 4m 넘게 헛되이 막힌 셈입니다.

     여덟 조각으로 둥글게 두릅니다. big 은 안 줍니다 — big 은 **카메라도
     막는** 표시인데, 분수는 허리 높이라 카메라가 그 위를 지나가야 합니다. */
  {
    const RF = 10.3, SEG = 8;
    for (let i = 0; i < SEG; i++) {
      const a2 = (i / SEG) * Math.PI * 2;
      const w = 2 * RF * Math.tan(Math.PI / SEG);
      solid(Math.cos(a2) * RF * .78, Math.sin(a2) * RF * .78, w, RF * .62, -a2, false);
    }
  }
     ...
     길목(x 0 축)은 비웁니다. */
  {
    ...
    /* 벤치 넷 — 네 사분면 주머니에서 분수를 **바라보게**. 분수 고리
       (반지름 15.2)와 두 축을 다 비켜 앉습니다. 광장 벤치가 바깥을
       보고 있으면 앉을 이유가 없습니다. */
    [[-16, -16], [16, -16], [-16, 16], [16, 16]].forEach(([x, z]) => {
      const ry = Math.atan2(-x, -z);
      benchOut(d, x, z, ry);
      solid(x, z, 2.4, 1.2, ry, false);
      /* 쓰레기통은 벤치 곁에 — 앉은 자리에서 손이 닿아야 쓰레기통입니다 */
      binOut(d, x + (x < 0 ? 2.4 : -2.4), z + (z < 0 ? -1.6 : 1.6));
    });

    /* 화단 여섯 — 넷은 네 귀퉁이(광장이 네모라는 것을 모서리에서
       말합니다), 둘은 남쪽 진입부에서 축을 **좌우로 끼웁니다.**
       정문에서 들어오면 이 둘 사이를 지나 분수로 가게 됩니다 —
       광장이 넓을수록 어디로 걸어야 할지를 바닥이 말해 줘야 합니다. */
    [[-24, -25], [24, -25], [-24, 24], [24, 24],
     [-13.5, 14], [13.5, 14]].forEach(([x, z]) => {
      flowerBed(d, x, z, 2.2); solid(x, z, 4.4, 4.4, 0, false);
    });
    ...
    /* 남쪽 벤치 둘 — 들어오는 사람을 마주 보게. 광장 남쪽 절반이
       통째로 비어 있어서 "아직 안 만든 데" 로 보였습니다. */
    [[-22, 20], [22, 20]].forEach(([x, z]) => {
      const ry = Math.atan2(-x, -z);
      benchOut(d, x, z, ry); solid(x, z, 2.4, 1.2, ry, false);
    });

    /* 이정표 하나 — 정문에서 들어와 처음 서는 자리. 축(x -11~11)은
       비우고 그 바로 옆에 세웁니다. 팔의 색이 그 건물의 지붕색입니다 —
       signpost 는 [색, 방향] 만 받고 글자는 안 그립니다. */
    signpost(d, 13, 24, -.5, [[0x3F6BA8, 0], [0x2C8C7E, 2.2], [0x43A05C, 4.0]]);
    solid(13, 24, .8, .8, 0, false);
    /* 소풍 자리 하나 — 남서 주머니. 광장에 "머물 수 있다" 를 말하는
       것은 벤치보다 이쪽이 큽니다. */
    picnicSet(d, -21, 12, .4);
    solid(-21, 12, 3.0, 2.2, .4, false);
  }

  /* --- 정문 · 야외 구역은 plan.js 가 세웁니다 ---
     여기 있던 것을 전부 걷어냈습니다. 부지를 네모로 다시 짜면서
     plan.js 가 정문 · 트랙 · 못을 새 자리에 세웠는데, 이 파일이 옛
     극좌표 자리에 **같은 것을 한 벌 더** 세우고 있었습니다 —
     정문이 둘, 트랙이 둘, 못이 둘. 광장 옆 20칸 거리에 또 다른 정문이
     서 있었으니 어디가 입구인지 알 수 없는 게 당연합니다.

     동아리 거리만 남깁니다. 학생회관 옆이 제자리라 그리로 옮깁니다. */
  clubStreet(g, ctx, 155, 30);

  /* --- 북서 잔디마당 — 소풍 자리 --- */
  /* 소풍 자리는 트랙 **바깥**(산책로 너머)으로 밀립니다. 안쪽 셋 중
     (-26,-22) 만 남기고 나머지는 트랙 연석과 겹쳤습니다. */
  /* 소풍 자리 · 그루터기는 옛 트랙 옆이었습니다. 트랙이 부지 반대편으로
     갔으니 여기 남으면 잔디 한복판에 상이 떠 있는 꼴입니다. */
  /* 깃대는 트랙이 자기 것으로 둘 세웁니다(trackField). 여기 있던 (-17,-13)
     은 트랙 인필드 한복판이라 뺍니다. 동쪽 짝은 그대로 둡니다. */
  /* 깃대와 산울타리도 옛 트랙에 딸린 것이라 뺍니다 */

  /* --- 남서 잔디마당 — 상점 앞 광장 --- */
  /* 상점 앞 소품도 뺍니다 — 상점이 (-16, 58) 로 갔는데 소품만 옛 자리인
     반지름 25 에 남아 있었습니다. 화단 · 벤치가 잔디에 떠 있던 정체입니다. */
