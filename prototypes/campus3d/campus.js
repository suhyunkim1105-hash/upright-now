...
export function buildCampus(scene) {
  ...
  /* ── 광장 소품 ──
     ...
     **광장의 줄눈에 맞춰 놓습니다.** 세로 줄눈이 x ±8.5 · ±17,
     ...
     길목(x 0 축)은 비웁니다. */
  {
    /* 가로등 여섯 — 양 긴 변의 줄눈(x ±22) 위에 셋씩 */
    [-20, 0, 20].forEach((z) => [-22, 22].forEach((x) => {
      lampPost(g, x, z, 4.4); solid(x, z, .9, .9, 0, false);
    }));
    /* 벤치 넷 — 분수를 **바라보게** 대각선에 놓습니다. 광장 벤치가
       바깥을 보고 있으면 앉을 이유가 없습니다. */
    [[-11.5, -11.5], [11.5, -11.5], [-11.5, 11.5], [11.5, 11.5]].forEach(([x, z]) => {
      benchOut(g, x, z, Math.atan2(-x, -z));
      solid(x, z, 2.4, 1.2, Math.atan2(-x, -z), false);
    });
    /* 화단 넷 — 네 귀퉁이. 광장이 네모라는 것을 모서리에서 말합니다 */
    [[-21, -22], [21, -22], [-21, 23], [21, 23]].forEach(([x, z]) => {
      flowerBed(g, x, z, 2.2); solid(x, z, 4.4, 4.4, 0, false);
    });
    /* 쓰레기통 둘 · 자전거 거치대 둘 — 벤치 곁과 광장 남쪽 어귀 */
    binOut(g, -13.6, 12.2); binOut(g, 13.6, 12.2);
    [[-25, 25, 0], [25, 25, 0]].forEach(([x, z, ry]) => {
      bikeRack(g, x, z, ry, 4); solid(x, z, 3.6, 1.0, ry, false);
    });
    /* 이정표 하나 — 정문에서 들어와 처음 서는 자리. 축(x 0)은 비우고
       그 옆 줄눈에 세웁니다. 팔의 색이 그 건물의 지붕색입니다 —
       signpost 는 [색, 방향] 만 받고 글자는 안 그립니다. */
    signpost(g, 8.5, 22, -.5, [[0x3F6BA8, 0], [0x2C8C7E, 2.2], [0x43A05C, 4.0]]);
    solid(8.5, 22, .8, .8, 0, false);
  }
