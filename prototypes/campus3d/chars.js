...
/* ══ 몸 비례 ══
   서 있는 렌더 넷(거북이·기린·펭귄·개구리)을 재 보면 이렇습니다.
...
/* ══ 종마다 다른 몸 ══
   ...
   넷이 다 같은 실루엣으로 나왔고, 어느 것도 레퍼런스와 안 맞았습니다.
...
/* ══ 옷 염색 여덟 ══
   ...
   이 여덟은 네 종 털 위에서 전부 옷으로 읽히는 색입니다. 검정은
   펭귄에서, 밝은 초록은 거북이·개구리에서,
...
/* ══════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════ */
const SPRITE_FILE = {
  거북이: 'turtle', 기린: 'giraffe', 개구리: 'frog', 펭귄: 'penguin',
};
...
function buildChar(parent, species, fit, opt) {
  ...
  torso.position.y = TORSO_Y; g.add(torso); parts.torso = torso;
  {
    /* 옆선을 종의 높이·폭으로 다시 그립니다. 비율은 유지하고 값만
       표에서 읽으므로, 기린은 좁고 길게 · 펭귄은 넓고 낮게 나옵니다. */
  ...
  const HAT_FIT = { 개구리: [.5, -.16, 1.05], 기린: [.46, -.02, 1.0] };
