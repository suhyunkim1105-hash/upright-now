...
const BODY = {
  /* 기린만 전체가 큽니다. 목이 길면 키가 커지는 게 당연하고,
     레퍼런스에서도 혼자 머리 하나만큼 큽니다. 같은 키로 맞추면
     목을 넣어도 기린으로 안 보입니다. */
...
function eyes(p, y, z, r, gap, opt = {}) {
  ...
  [-gap, gap].forEach((dx, i) => {
    ...
    /* 앞선 판단이 틀렸습니다.
       ...
       저해상도 교실 스틸에서는 눈이 작은 점으로 보여 .62 로 줄였는데,
       원본(char-penguin-3d.webp, 1856×2304)을 재 보니 **눈 지름이 머리
       폭의 18%** 이고 표면이 젖은 유리알입니다. 멀리서 점으로 보였을
       뿐입니다. 크게, 그리고 매끈하게 되돌립니다. */
    made.push(ell(p, r * 1.05, M(SK.ink, .05, { metalness: .02 }), dx, y, z, 1, 1.04, .72));
    /* 하이라이트는 **크게 하나** — 개구리 눈이 유일하게 합격한 이유가
       이 큰 빛점이었습니다. 작은 점 두 개는 멀리서 사라집니다. */
    /* 원본의 빛점은 왼쪽 위에 **하나, 크고 또렷하게** 있습니다 */
    ell(p, r * .30, M(SK.white, .04), dx - s * r * .34, y + r * .38, z + r * .5, 1, 1, .5);
  });
...
function cheeks(p, x, y, z, r = .1) {
  /* 볼터치도 원본에는 **있습니다** — 눈 바로 아래 바깥쪽에 분홍 타원
     둘. 저해상도 스틸에서 안 보여 없앴던 것을 되돌립니다. */
...
const HEADS = {
  거북이(h, C) {
    ...
    /* 거북이 — 얼굴이 매끈한 돔이라 눈이 유일한 이목구비입니다.
       크지 않되 **넓게** 벌어져 순한 인상을 만듭니다. */
    eyes(h, .10, .43, .098, .22);
    smile(h, -.16, .47, .1);
  ...
  기린(h, C) {
    /* 머리를 앞뒤로 늘립니다 — 기린 얼굴은 공이 아니라 길쭉한 덩어리입니다 */
    ell(h, .46, M(C.skin), 0, .03, .02, .86, 1.0, 1.16);
    /* 주둥이 — 작게, 색은 얼굴보다 아주 조금만 밝게 */
    ell(h, .5, M(C.snout, .55), 0, -.17, .3, .5, .38, .42);
    ell(h, .05, M(SK.ink, .3), -.07, -.1, .48, 1, .8, .6);
    ell(h, .05, M(SK.ink, .3), .07, -.1, .48, 1, .8, .6);
    smile(h, -.24, .44, .09);
    /* 기린 — 얼굴이 앞으로 길어서 눈이 상대적으로 작고 **옆쪽**에
       붙습니다. 정면 한복판에 크게 두면 초식동물로 안 보입니다. */
    eyes(h, .17, .36, .078, .27);
  ...
  알파카(h, C) {
    ...
    smile(h, -.22, .42, .08);
    /* 알파카 — 털에 파묻혀 **점처럼** 보입니다. 일곱 중 가장 작습니다. */
    eyes(h, .11, .40, .072, .20);
    cheeks(h, .28, -.1, .38);
  },
  햄스터(h, C) {
    /* 옆으로 넓은 얼굴 — 큰 귀와 함께 햄스터의 윤곽을 만듭니다 */
  ...
  고슴도치(h, C) {
    ...
    /* 열둘로는 뒤통수에 몇 가닥 얹은 정도였습니다. 레퍼런스의 고슴도치는
       **정수리부터 뒷목까지 통째로** 가시입니다 — 얼굴만 빼고 다 덮습니다.
       구면 좌표로 뒤쪽 반구를 훑어 심으면 빈 곳이 안 생깁니다. */
    let i = 0;
    for (let ring = 0; ring < 4; ring++) {
      const phi = .32 + ring * .42;                 // 정수리에서 아래로
      const n = [5, 7, 8, 7][ring];
      for (let k = 0; k < n; k++) {
        /* 뒤쪽 반원만 — 얼굴(전방 z+)은 비웁니다 */
        const th = Math.PI * (.30 + (k / (n - 1)) * 1.40);
        const r0 = .46;
        const x = Math.sin(phi) * Math.cos(th) * r0;
        const y = Math.cos(phi) * r0 - .01;
        const z = Math.sin(phi) * Math.sin(th) * -r0 * .95 + .04;
        quill(x, y, z, .16 - ring * .008, i++);
      }
    }
    ell(h, .5, M(C.snout, .55), 0, -.16, .28, .44, .34, .4);
    ell(h, .06, M(SK.ink, .3), 0, -.07, .46, 1.1, .85, .7);
    smile(h, -.24, .42, .08);
    /* 고슴도치 — 뾰족한 주둥이 위의 작고 검은 눈 */
    eyes(h, .13, .40, .082, .175);
  ...
  펭귄(h, C) {
    ...
    /* 펭귄 — 흰 가면 위에 크고 동그랗게. 원본에서 잰 값이 머리 폭의
       18% 라 일곱 중 개구리 다음으로 큽니다. */
    eyes(h, .10, .48, .135, .195);
