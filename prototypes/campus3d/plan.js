...
export const BUILDINGS = [
  /* ══ 축과 중앙 지구 ══
     정문(0,130) → 진입 마당 → 대로 → 사색의 광장 → 대계단 → 본관(0,-42).
     축이 **무언가의 얼굴에서 끝나야** 축입니다. 전 판은 분수 한가운데서
     끊겨 잔디로 사라졌습니다.

     본관 하나로는 폭 18칸이라 22칸짜리 축의 머리로 모자랍니다. 좌우에
     서관·동관을 붙여 70칸짜리 한 덩어리로 만들고 가운데를 높입니다 —
     실제 본관들이 별관을 거느리는 이유가 이것입니다. */
  { n: '본관',       enter: 'mainHall', x: 0, z: -42, face: 'S', s: 1.9, w: 9.6, d: 6.2, front: 2.5 },
  { n: '본관 서관',  kind: 'admin', x: -26, z: -46, face: 'S', w: 22, d: 12, h: 12 },
  { n: '본관 동관',  kind: 'admin', x:  26, z: -46, face: 'S', w: 22, d: 12, h: 12 },

  /* 광장의 벽. 도서관과 학생회관이 **마주 봅니다** — 면 사이 49칸,
     높이 15칸이라 위요비 1:3.3. 전 판은 가장 가까운 건물이 50칸 밖이라
     1:6.7 이었고, 그래서 광장이 아니라 잔디에 난 원이었습니다.
     국제대학·학생문화관이 그 벽을 북쪽으로 이어 광장을 닫습니다. */
  { n: '도서관',     enter: 'library', x: -30, z: 2, face: 'E', s: 1.9, w: 10.0, d: 6.4, front: 2.4 },
  { n: '학생회관',   enter: 'union',   x:  30, z: 2, face: 'W', s: 1.6, w: 9.0, d: 5.6, front: 1.9 },
  { n: '국제대학',   kind: 'atrium', x: -32, z: -20, face: 'E', w: 26, d: 12, h: 13 },
  { n: '학생문화관', kind: 'hall',   x:  32, z: -20, face: 'W', w: 26, d: 12, h: 13 },

  /* ══ 인문사회 지구 — 서쪽 안뜰 44×44, 동쪽으로 엽니다 ══
     세 동이 ㄷ자로 세 면을 채우고 한 면만 광장 쪽으로 틉니다. 안뜰은
     **건물이 만드는 방**이지 건물 사이에 남은 자리가 아닙니다. */
  { n: '인문대학',     kind: 'brick', x: -92, z: -34, face: 'S', w: 48, d: 12, h: 13 },
  { n: '사회과학대학', kind: 'slab',  x: -118, z: -6, face: 'E', w: 48, d: 12, h: 13 },
  { n: '경영대학',     kind: 'brick', x: -92, z:  22, face: 'N', w: 48, d: 12, h: 12 },
  { n: '외국어대학',   kind: 'slab',  x: -128, z: -54, face: 'E', w: 34, d: 12, h: 12 },
  { n: '대학원',       kind: 'admin', x: -58, z: -60, face: 'E', w: 30, d: 13, h: 15 },

  /* ══ 자연공학 지구 — 북쪽 안뜰 44×38, 남쪽으로 엽니다 ══ */
  { n: '공과대학',     kind: 'wing',      x: -2, z: -106, face: 'S', w: 58, d: 12, h: 14 },
  { n: '전자정보대학', kind: 'slab',      x: -30, z: -86, face: 'E', w: 42, d: 12, h: 13 },
  { n: '응용과학대학', kind: 'tower_lab', x:  26, z: -86, face: 'W', w: 42, d: 12, h: 14 },
  { n: '생명과학대학', kind: 'tower_lab', x:  56, z: -90, face: 'S', w: 26, d: 12, h: 13 },
  { n: '약학대학',     kind: 'slab',      x:  95, z: -76, face: 'SW', w: 26, d: 12, h: 12 },
  { n: '간호과학대학', kind: 'brick',     x:  62, z: -62, face: 'W', w: 26, d: 12, h: 12 },

  /* ══ 예술 · 생활 지구 — 동쪽 안뜰 44×46, 서쪽으로 엽니다 ══
     남쪽 면은 작은 두 동이 나란히 맡습니다. 그 사이 11칸이 안뜰로
     드는 샛길이 됩니다 — 실제 캠퍼스의 동 사이 통로가 그것입니다. */
  { n: '예술디자인대학', kind: 'atrium',  x:  96, z: -34, face: 'S', w: 48, d: 12, h: 13 },
  { n: '박물관',         kind: 'library', x: 122, z:  -6, face: 'W', w: 48, d: 12, h: 12 },
  { n: '미니게임관',  enter: 'arcade', x:  84, z: 22, face: 'N', s: 1.6, w: 8.4, d: 5.6, front: 1.9 },
  { n: '동아리 상점', enter: 'shop',   x: 108, z: 22, face: 'N', s: 1.6, w: 8.0, d: 5.2, front: 1.8 },

  /* ══ 가장자리 — 체육 · 기숙사 · 진입 마당 ══
     가운데는 조이고 가장자리는 풉니다. 전 판은 최근접 이웃 거리가
     최소 30 · 최대 55 로 **거의 고르게** 흩어져 있었고, 그 균일함이
     캠퍼스가 아니라 마을로 읽히던 정체였습니다. */
  { n: '체육관',    kind: 'gym', x: -130, z: 40, face: 'E', w: 30, d: 18, h: 11 },
  { n: '기숙사',    enter: 'dorm', x: 116, z: -56, face: 'SW', s: 1.6, w: 8.2, d: 5.4, front: 1.9 },
  { n: '제1기숙사', kind: 'hall_res', x: 118, z: -100, face: 'S', w: 26, d: 10, h: 15 },
  { n: '제2기숙사', kind: 'hall_res', x:  84, z: -104, face: 'S', w: 26, d: 10, h: 15 },
  { n: '평화의전당', kind: 'hall',  x:  52, z: 96, face: 'W', w: 30, d: 20, h: 16 },
  { n: '입학처',     kind: 'admin', x: -47, z: 96, face: 'E', w: 24, d: 14, h: 11 },
];
...
/* ══════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════ */
export const FIELDS = [
  /* 체육 지구 — 서쪽 가장자리에 한 덩어리로 */
  { t: 'track',  x: -84, z:  62, w: 68, d: 38, ry: 0 },
  { t: 'tennis', x: -104, z: 94, w: 40, d: 22, ry: 0 },

  /* 못과 야외극장 — 동쪽 정원 */
  { t: 'pond',   x: 120, z: 70, w: 42, d: 30, ry: 0 },
  { t: 'amphi',  x:  84, z: 58, w: 26, d: 26, ry: 0 },

  /* 주차장 둘 — 정문 옆과 기숙사. 셋은 많았습니다 */
  { t: 'lot',    x:  94, z:  98, w: 36, d: 22, ry: 0 },
  { t: 'lot',    x: 124, z: -78, w: 26, d: 20, ry: 0 },
];
...
export const ROADS = [
  /* ① 축 — 폭 22. 하나뿐이고 곧습니다.
        정문에서 광장 남단까지. 광장 안은 포장이라 길을 깔지 않습니다. */
  { w: 22, pts: [[0, 130], [0, 104], [0, 74], [0, 48], [0, 34]] },

  /* ② 순환로 — 폭 11. 정문 안에서 좌우로 갈라져 담을 따라 돕니다.
        차 · 주차 · 서비스가 여기로만 다니면 안쪽이 온전히 보행이 됩니다. */
  { w: 11, pts: [[0, 120], [-56, 120], [-112, 118], [-146, 104], [-150, 50],
                 [-150, -20], [-148, -72], [-138, -114], [-70, -120], [-20, -120]] },
  { w: 11, pts: [[0, 120], [56, 120], [112, 118], [146, 104], [150, 50],
                 [150, -20], [148, -72], [138, -114], [70, -120], [-20, -120]] },

  /* ③ 보행로 — 폭 6. 안뜰과 안뜰을 잇습니다.
        곡선을 폈습니다. 캠퍼스 길이 곧은 것은 건물이 직각이기 때문입니다 —
        굽은 길은 공원의 것이고, 굽히면 어느 길이 중심인지 사라집니다. */
  { w: 6, pts: [[-14, 26], [-42, 28], [-58, 22], [-62, 8]] },
  { w: 6, pts: [[14, 26], [42, 28], [58, 22], [62, 8]] },
  { w: 6, pts: [[0, -50], [0, -58], [0, -70]] },
  { w: 6, pts: [[-84, 34], [-84, 44]] },
  { w: 6, pts: [[36, 32], [58, 44], [74, 52]] },
];
...
/* ══════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════ */
export const COURTS = [
  { x: -90, z:  -6, w: 44, d: 44 },   // 인문사회
  { x:  -2, z: -81, w: 44, d: 38 },   // 자연공학
  { x:  94, z:  -5, w: 44, d: 45 },   // 예술 · 생활
];
