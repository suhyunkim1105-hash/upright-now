...
export const ROOMS = {
  ...
  mainhall(g) {
    ...
    R.window3(g, -7.6, 2.7, 16, 2.1, 2.1);
    R.window3(g, 9.0, 2.7, 16, 2.1, 2.1);

    /* 교탁 구역 — 세로 통로를 비켜 양옆으로 */
    R.podium(g, -5.4, -6.6, .18);
    R.desk(g, 4.4, -6.6, 0, 2.4, 1.0, .9);
    R.books(g, 3.8, 1.06, -6.6, 2);
    R.laptop(g, 5.1, 1.06, -6.6);

    /* 강의석 두 줄 × 세 칸 — 줄 간격 2.9, 줄 사이 0.9 가 남습니다 */
    [-3.6, -0.7, 2.2].forEach((z, ri) => {
      [-7.0, 7.0].forEach((x, ci) => {
        R.desk(g, x, z, 0, 5.2, 1.1, .78);
        [-1.9, -0.65, 0.65, 1.9].forEach((dx) => R.chair(g, x + dx, z + 1.2, Math.PI, 0x9BB4D6));
        R.books(g, x - 2.0, .87, z - .1, 2);
        R.books(g, x + 2.0, .87, z + .1, ((ri + ci) % 2) + 1);
        if ((ri + ci) % 2 === 0) R.laptop(g, x + .7, .87, z - .1);
        if ((ri + ci) % 3 === 1) R.deskClutter(g, x - .6, .87, z);
      });
    });

    /* 앞쪽 — 사물함 · 벤치 · 겹친 의자 */
    R.lockers(g, -10.4, -1.2, Math.PI / 2, 4, 0x8AB4CE);
    R.lockers(g, -10.4, 6.6, Math.PI / 2, 4, 0x8AB4CE);
    /* 벤치도 강의실 **안쪽**(칠판 쪽)을 봅니다 */
    R.bench(g, -6.4, 6.8, Math.PI, 3.6);
    R.bench(g, 6.4, 6.8, Math.PI, 3.6);
    R.stackChairs(g, 9.8, 6.4, -.4, 5);
    R.stackChairs(g, 9.8, 5.9, -.2, 4, 0xE8935A);
    R.cooler(g, 9.9, -6.2, Math.PI);
    R.aFrame(g, -9.6, 3.0, .5, 0xF2C14E);
    R.bin(g, -9.9, -3.2, 0x5B84C4); R.bin(g, 9.9, 1.9, 0x5E8C6A);
    R.radiator(g, 5.4, -7.5, 0, 2.6);
    R.clock(g, 9.6, 3.6, -7.82);
    R.poster(g, 9.6, 2.2, -7.82, 0, 1.0, 1.3, 0xF2C14E);
    R.banner(g, -10.72, 3.6, 0.6, Math.PI / 2, 3.4, .95, 0x3FB3A2);
    R.wallShelf(g, -10.62, 3.0, -4.4, Math.PI / 2, 1.8);
    [-7.0, 0, 7.0].forEach((x) => { R.striplight(g, x, -3.0, 4.0, 3.4); R.striplight(g, x, 2.6, 4.0, 3.4); });
    R.rug(g, 0, -6.4, 4.4, 2.2, 0x4E6E8C, 0xDCE6F0);
    R.plant(g, 10.3, -3.0, 1.05); R.plant(g, -10.3, 7.2, 1.1);
