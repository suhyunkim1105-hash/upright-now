...
export const ROOMS = {
  ...
  library(g) {
    ...
    /* ── 뒤벽 서가 ── 벽 하나를 통째로 책으로 채웁니다. 도서관이
       도서관으로 읽히는 첫 조건입니다. */
    for (let i = 0; i < 21; i++) R.shelf(g, -21.5 + i * 2.05, -16.3, 0, 1.9, 4.6);
    /* 좌우 벽 서가 — 창 사이사이 */
    [-14.0, -11.0, 11.0, 14.0].forEach((z) => {
      R.shelf(g, -22.9, z, Math.PI / 2, 1.9, 4.0);
      R.shelf(g, 22.9, z, -Math.PI / 2, 1.9, 4.0);
    });
    /* 방 가운데 등 서가 두 줄 — 넓어진 만큼 안쪽에도 책이 있어야
       "서가 사이를 지나 자리로 간다" 가 됩니다. */
    [-7.0, 7.0].forEach((cx) =>
      [-11.5, -9.3].forEach((cz) => R.shelf(g, cx, cz, 0, 1.9, 4.0)));

    /* ── 열람 탁자 여섯 ── 두 줄 × 셋. 여덟 자리씩 마흔여덟.
       세로 통로(x ±2.6)를 비켜 좌우로 갈라 놓습니다. */
    [[-13.4, -5.4], [-13.4, 1.6], [-13.4, 8.6], [-13.4, 13.4],
     [13.4, -5.4], [13.4, 1.6], [13.4, 8.6], [13.4, 13.4]].forEach(([cx, cz], k) => {
    ...
    R.globe(g, -4.4, 1.06, 14.4);
    R.bookCart(g, -2.2, 13.6, .3);
    R.bookCart(g, 4.6, 15.0, -.4);
    R.sofa(g, 8.4, 15.2, Math.PI, 4.2, 0x7FA8C4);
    R.lowTable(g, 8.4, 13.4, 0, 2.2, 1.2);
    R.books(g, 8.1, .56, 13.4, 2);
    R.mag(g, 1.6, 15.4, 0);
    R.plant(g, 22.2, 7.4, 1.15);
    R.plant(g, -22.2, 7.4, 1.0);
    R.plant(g, 22.2, -7.4, 1.0);
    R.plant(g, -22.2, -7.4, 1.0);
    ...
    [-13.4, 0, 13.4].forEach((x) => [-5.4, 1.6, 8.6, 13.4].forEach((z) => R.pendant(g, x, z, 5.8)));
    R.rug(g, 8.4, 14.0, 7.4, 4.2, 0xB08AB0, 0xF2E4E8);
    R.bin(g, -4.6, 15.4); R.bin(g, 4.4, -15.2);
    R.aFrame(g, -20.0, 10.0, .5, 0x2C8C7E);
  },
  ...
  mainhall(g) {
    ...
    [-7.6, -4.2, -.8, 2.6, 6.0, 9.4].forEach((z, ri) => {
      [-12.4, 12.4].forEach((x, ci) => {
        R.desk(g, x, z, 0, 8.4, 1.2, .78);
    ...
    [-12.4, 0, 12.4].forEach((x) => [-7.6, -.8, 6.0].forEach((z) => R.pendant(g, x, z, 4.8)));
  ...
  union(g) {
    ...
    /* 명예의 전당 — 창구가 아니라 학교 로고 액자가 걸린 벽입니다. */
    box(g, 6.8, 2.9, .20, .08, M(0x28313D, .72), -7.1, 2.55, -7.28);
    box(g, 5.9, .34, .24, .08, M(0xF4E6B8, .5), -7.1, 3.66, -7.14);
    [-9.3, -8.2, -7.1, -6.0, -4.9].forEach((x, i) => {
      const c = [0xD4AF37, 0xAFC3D5, 0xB77A4A, 0x2DD4BF, 0x9B7BD4][i];
      box(g, .88, 1.42, .16, .06, M(0x171C24, .72), x, 2.66, -7.08);
      box(g, .68, 1.18, .12, .04, M(0xEEF3F8, .42), x, 2.66, -6.98);
      cyl(g, .22, .25, .10, 12, M(c, .54), x, 2.88, -6.88).rotation.x = Math.PI / 2;
      box(g, .38, .08, .11, .03, M(0x475568, .44), x, 2.42, -6.88);
      [0, 1, 2].forEach((k) => box(g, .46 - k * .08, .05, .11, .02,
        M(0x7C8796, .35), x, 2.26 - k * .10, -6.88));
    });
    R.stanchion(g, -9.8, -4.8, -4.4, -4.8);

    /* 왼쪽 두 번째 책상은 학생증 발급 전용. 책상 위 카드 모형도 같이 둡니다. */
    R.counter(g, -4.4, -5.9, 0, 3.4, IN.woodDark);
    R.counterTop(g, -4.4, 1.1, -5.7);
    R.chair(g, -4.4, -6.9, 0, 0x9BB4D6);
    R.board(g, -4.4, 3.0, -7.32, 2.6, 1.6);
    box(g, .76, .48, .06, .06, M(0xDCEAF5, .5), -4.4, 1.28, -5.42);
    box(g, .20, .20, .07, .05, M(0x2DD4BF, .5), -4.62, 1.32, -5.37);
    [0, 1, 2].forEach((i) => box(g, .30 - i * .04, .04, .07, .02,
      M(0x526174, .4), -4.18, 1.40 - i * .09, -5.37));

    /* 학생식당 — 배식 레일·메뉴보드·식판·대기줄이 한 덩어리로 보입니다. */
    R.trayCounter(g, 6.4, -5.9, 0, 5.4);
    R.blackboard(g, 6.4, 3.15, -7.28, 5.2, 1.55);
