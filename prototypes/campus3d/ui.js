  ...
  return {
    ...
    html: '<div class="mp">'
      ...
      /* 이름 — 원래 옷장(C)에 있었습니다. 옷장은 "입는 것"의 집이고
         이름은 나를 정하는 것이라, 학교 위 첫 칸으로 옮깁니다. */
      + lbl('user', '이름')
...
/* ══════════════════════════════════════════════════════════
   건물 안내 — 실내 안내 NPC 에게 말을 걸면 뜨는 카드들.
   말풍선 세 줄로 하던 안내를 그림 카드로 폅니다 — 건물마다 서너 장,
   "여기서 뭘 할 수 있나" 만 말하고 사용법 상세는 그 자리에 가면
   E 프롬프트가 이어받습니다.
   ══════════════════════════════════════════════════════════ */
export const ROOM_GUIDE = {
  library: [
    ['seat', '', '열람석 자세 세션', '아무 열람석 앞에서 E — 10초 동안 기준을 잡고 시작해요'],
    ['music', 'sky', '집중 소리', 'MY → 설정에서 도서관 ASMR 을 고를 수 있어요'],
    ['book', 'lemon', '책 정리 미니게임', '오른쪽 반납 책수레 앞에서 E'],
    ['clock', 'lilac', '앉은 기록', '앉은 시간과 회복은 마이페이지 내 기록에 쌓여요'],
  ],
  mainhall: [
    ['cal', '', '오늘 강의', '앞 교단에서 오늘 강의와 다음 강의를 봐요'],
    ['seat', 'sky', '강의석 자세 세션', '모든 강의석 앞에서 E — 도서관과 같은 규칙이에요'],
    ['bell', 'lemon', '강단 스피커', '집중 소리를 크게 틀 수 있어요'],
  ],
  union: [
    ['chart', '', '명예의 전당', '왼쪽 갤러리 벽 — 학교 순위가 걸려 있어요'],
    ['cup', 'sky', '오늘의 학식', '배식대에서 학교별 오늘 메뉴를 봐요'],
    ['user', 'lemon', '학생증', '학생증 발급대에서 내 카드를 확인해요'],
    ['coin', 'lilac', '음료 자판기', '코인으로 음료를 뽑아요'],
    ['sofa', 'peach', '라운지', '쉬는 의자에는 실제로 앉을 수 있어요'],
  ],
  arcade: [
    ['run', '', '오락기 넷', '러너 · 짝 맞추기 · 셋 지우기 · 동물 합치기'],
    ['ticket', 'sky', '희귀 선물 뽑기', '이벤트 뽑기에서 한정 가구가 나와요'],
    ['coin', 'lemon', '코인으로 놀아요', '코인은 자세 세션으로 모아요'],
  ],
  shop: [
    ['shirt', '', '옷 가게', '입어 보고 마음에 들면 코인으로 사요'],
    ['sofa', 'sky', '가구 가게', '기숙사 방 꾸미기에 쓸 가구를 팔아요'],
    ['egg', 'lemon', '알 가게', '알에서 새 종이 깨어나요 — 옷장에서 갈아타요'],
  ],
};
/** ctx: { room, name, role } */
export function roomGuide(ctx) {
  const cards = ROOM_GUIDE[ctx.room] || [];
  return {
    tag: ctx.role || '안내', title: ctx.name ? `${ctx.name}의 안내` : '안내',
    html: `<div class="cg">${cards.map(([ic, tone, nm, ds]) => cb(ic, tone, nm, ds)).join('')}</div>`
      + hlp('각 자리 앞에 서면 E 안내가 떠요 · Esc 로 닫기'),
  };
}
...
/* ══════════════════════════════════════════════════════════
   ...
   **조작**을 알려 주지 이 세계가 무엇인지는 말해 주지 않습니다.
   ...
   ══════════════════════════════════════════════════════════ */
/* 화면 구석의 힌트 줄(투토리얼 스트립)은 은퇴했습니다 — 처음 안내와
   힌트가 **동시에 두 개** 떠서 어느 쪽을 읽어야 하는지부터 헷갈렸습니다.
   조작은 여기 둘째 장이 말하고, 자세한 표는 마이페이지 → 설정 → 단축키에
   있습니다. */
const TOUR = [
  { ic: 'pic-map', tone: 'sky', t: '섬 하나가 캠퍼스예요',
    p: '건물 여섯 채와 운동장·동아리 거리가 있습니다. M 을 누르면 지도가 열리고, 건물을 누르거나 이름 앞 숫자를 누르면 그 앞으로 갑니다.' },
  { ic: 'pic-key', tone: 'lilac', t: '조작은 이렇게',
    p: 'WASD 로 걷고 Shift 로 뜁니다. 화면을 끌면 시점이 돌고, 휠로 멀리·가까이, Tab 은 1인칭. 문·의자·사람 앞에서는 E 하나면 다 됩니다.' },
  { ic: 'pic-seat', tone: '', t: '앉으면 시작됩니다',
    p: '도서관이나 본관 의자 앞에서 E. 처음 10초 동안 지금 앉은 모습을 기준으로 잡고, 웹캠이 목 각도를 봐요. 무너지면 캐릭터도 같이 굽습니다.' },
