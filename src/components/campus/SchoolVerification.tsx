...
import { isSchoolEmailDomain, normalizeSchoolEmail } from '@/features/campus/verification'
...
export function SchoolVerification({ schoolId, schoolName }: { schoolId: string; schoolName: string }) {
  ...
  const finishVerification = useCallback(async () => {
    ...
    try {
      ...
      console.error('[campus verification] confirmation failed', error)
  ...
  const requestCode = async () => {
    ...
    if (!isSchoolEmailDomain(schoolId, normalized.domain)) {
      return setMessage(MESSAGE.domain_mismatch)
    }
    const repository = getCampusRepository()
    ...
    try {
      ...
      console.error('[campus verification] request failed', error)
      setMessage(MESSAGE.network_error)
    } finally {
      setBusy(false)
    }
